  ...
  useEffect(() => {
    const processPayment = async () => {
      try {
        ...
        // 3. 결제 완료 처리
        const response = await fetch('/api/shop/feathers/purchase', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            paymentId: paymentId,
          ...
          {status === 'error' && (
            <>
              ...
              <div className="space-y-3">
                <button
                  onClick={() => router.push('/shop/feathers')}
                  className="w-full bg-amber-600 text-white py-3 rounded-lg font-medium hover-hover:hover:bg-amber-700 transition-colors"
                >
                  다시 시도
                </button>
                <button
